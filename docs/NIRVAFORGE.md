...
6. [Security Architecture](#6-security-architecture) · [6.1 Compliance Framework](#61-compliance-framework-phase-9)
...
## 6.1 Compliance Framework (Phase 9)

รองรับ **ISO 9001 · 14001 · 45001 · 27001 · 10002 · 10004 · PDPA · GDPR** —
หัวใจคือ **ไม่ทำแยกกัน 8 ระบบ แต่รวมเป็น Integrated Management System (IMS) เดียว**

### ข้อมูลใดเก็บร่วมกันได้ / ซ้ำซ้อน

ทุกมาตรฐาน ISO ใช้โครงสร้างกลางเดียวกัน (**Annex SL / High-Level Structure**):
Context → Leadership → Planning → Support → Operation → Performance Evaluation →
Improvement จึงมี element ที่ **ใช้ร่วมกันได้ทันที** (เก็บครั้งเดียว ใช้ทุก
มาตรฐาน):

| Element ใช้ร่วม (shared core) | ใช้กับมาตรฐานใด |
|------------------------------|------------------|
| Document & Record Control | ทุก ISO |
| Internal Audit | ทุก ISO |
| Management Review | ทุก ISO |
| Corrective Action / Non-conformance | ทุก ISO |
| Risk & Opportunity Register | ทุก ISO (+ ผูก Risk L8) |
| Competence / Training records | ทุก ISO (+ People layer) |
| Objectives & KPI | ทุก ISO |

**ส่วนที่เป็น standard-specific (ไม่ซ้ำ):**

| มาตรฐาน | จุดเฉพาะ |
|---------|----------|
| ISO 9001 (Quality) | process map, ความพึงพอใจลูกค้า |
| ISO 14001 (Environment) | aspect/impact, ผูก **ESG layer** (carbon/waste/energy) |
| ISO 45001 (OH&S) | hazard, incident, ความปลอดภัยพนักงาน (สำคัญมากสำหรับธุรกิจทำความสะอาด) |
| ISO 27001 (InfoSec) | Statement of Applicability, controls → ผูก **Security §6** |
| ISO 10002 (Complaints) | complaint handling → ผูก Customer Support Agent |
| ISO 10004 (Customer Satisfaction) | การวัด/ติดตามความพึงพอใจ → ผูก CRM/BI |
| **PDPA / GDPR** | data-subject rights, consent, RoPA, breach notify, DPO |

> **PDPA ↔ GDPR ซ้ำซ้อนสูง** — ทั้งคู่ใช้ฐานเดียวกัน (records of processing,
> consent, สิทธิเจ้าของข้อมูล, แจ้งเหตุละเมิด) ต่างกันที่รายละเอียด/timeline →
> สร้าง **privacy framework เดียว** แล้ว map ความต่างเป็น config ต่อเขตอำนาจ

### รวมเป็น Framework กลางอย่างไร

```
        ┌──────── IMS CORE (เก็บครั้งเดียว ใช้ทุกมาตรฐาน) ────────┐
        │ Document Control · Internal Audit · Mgmt Review ·       │
        │ Corrective Action · Risk Register · Training · KPI      │
        └───────────────────────┬────────────────────────────────┘
                                 │  control แต่ละข้อ tag ว่าตอบมาตรฐานใด (many-to-many)
   ┌──────┬──────┬──────┬──────┬─┴────┬──────┬──────────┬──────────┐
   ▼      ▼      ▼      ▼      ▼      ▼      ▼          ▼
 9001  14001  45001  27001  10002  10004  PDPA       GDPR
                                          └── Privacy core เดียว, config ต่อเขต ──┘
```

**กลไก:** control 1 ข้อ map ได้หลายมาตรฐาน (many-to-many) → ทำ audit ครั้งเดียว
ได้หลักฐานตอบหลายมาตรฐานพร้อมกัน, ลดงานซ้ำ, ISO Agent (Phase 3) เป็นตัวรวบหลักฐาน
จาก event log อัตโนมัติ
