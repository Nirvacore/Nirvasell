...
c1.metric(t("turn.kpi_avg"), str(summary.get("avg_turnover",0)) + "x")
...
with tab_all:
    ...
    if not items:
        ...
        items_sorted = sorted(items, key=lambda x: x.get("turnover_rate",0), reverse=True)
        max_ratio = items_sorted[0].get("turnover_rate",1) if items_sorted else 1
        for item in items_sorted:
            ratio   = item.get("turnover_rate",0)
            doh     = item.get("doi",0)
            sku     = item.get("sku","?")
